...
Create (or refresh) LoRa distribution manifest(s).
  ...
  • walks   dist/       for regular files (skips names that start with '.' or '_')
  • keeps   dist/_serial.txt  with two counters:
        next_file_id=<int>
...
def parse_existing_ids(manifest_path: Path | None) -> Dict[str, int]:
    """
    Return a mapping  relative_path → file_id  from the previous manifest
    so we can keep IDs stable.  Relative path key matches exactly what
    appears in the manifest lines.
    """
    ...
    with manifest_path.open() as fp:
        ...
        for line in fp:
            ...
            if not line or line.startswith("#"):
                continue
            parts = line.split("\t")
            if active_section == "fw" and len(parts) >= 5:
                # firmware section: id, product, version, size, md5
                fid, product, ver = int(parts[0]), parts[1], parts[2]
                rel = f"firmware/{product}/{ver}.bin"
                ids[rel] = fid
            elif active_section == "files" and len(parts) >= 4:
...
def build_manifest() -> None:
    ...
    for prod, ver, size, md5, p in fw_entries:
        ...
        file_id = id_lookup.get(rel, next_file_id)
