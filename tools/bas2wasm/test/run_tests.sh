#!/bin/bash
# bas2wasm test runner
# Compiles all test .bas files and validates the output WASM.
# reject_*.bas files are expected to produce errors (nonzero exit).

set -u

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BAS2WASM="$SCRIPT_DIR/../bas2wasm"
TMPDIR=$(mktemp -d)
...
if [ ! -x "$BAS2WASM" ]; then
    echo "bas2wasm not found at $BAS2WASM — run 'make' first"
    exit 1
fi
...
echo "=== bas2wasm test suite ==="
echo ""

# --- Positive tests: should compile and validate ---
for src in "$SCRIPT_DIR"/*.bas; do
    [ -f "$src" ] || continue
...
for src in "$SCRIPT_DIR"/reject_*.bas; do
    [ -f "$src" ] || continue
    name=$(basename "$src" .bas)
    wasm="$TMPDIR/${name}.wasm"
    ...
    if [ $rc -ne 0 ]; then
        err=$(echo "$out" | grep -m1 -i "error" || echo "$out" | head -1)
...
# --- Also compile the firmware/data/*.bas production examples ---
echo "--- examples ---"
EXAMPLES_DIR="$SCRIPT_DIR/../../../firmware/data"
if [ -d "$EXAMPLES_DIR" ]; then
    for src in "$EXAMPLES_DIR"/*.bas; do
        [ -f "$src" ] || continue
        name=$(basename "$src" .bas)
        wasm="$TMPDIR/${name}.wasm"
