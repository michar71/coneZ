...
static const cfg_descriptor_t cfg_table[] = {
    ...
    CFG_ENTRY("lora",   "mode",         CFG_STR,   lora_rf_mode),
    // [fsk] -- FSK PHY params in their own section (matches the master's INI)
    CFG_ENTRY("fsk",    "bitrate",      CFG_FLOAT, fsk_bitrate),
    CFG_ENTRY("fsk",    "freqdev",      CFG_FLOAT, fsk_freqdev),
    CFG_ENTRY("fsk",    "rxbw",         CFG_FLOAT, fsk_rxbw),
    CFG_ENTRY_R("fsk",  "shaping",      CFG_INT,   fsk_shaping,    0, 4),
    CFG_ENTRY("fsk",    "whitening",    CFG_BOOL,  fsk_whitening),
    CFG_ENTRY("fsk",    "sync",         CFG_STR,   fsk_syncword),
    CFG_ENTRY_R("fsk",  "crc",          CFG_INT,   fsk_crc,        0, 2),
    // [system]
