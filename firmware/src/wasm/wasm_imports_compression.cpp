...
m3ApiRawFunction(m3_inflate_file_to_mem)
{
    ...
    // Stream decompressed chunks to WASM memory via helpers
    struct wasm_write_ctx ctx = { runtime, (uint32_t)dst_ptr, (size_t)dst_max, 0 };
    int result = inflate_stream(in_buf, in_size, wasm_write_cb, &ctx);
    free(in_buf);
...
m3ApiRawFunction(m3_inflate_mem)
{
    ...
    // Stream decompressed chunks to WASM memory via helpers
    struct wasm_write_ctx ctx = { runtime, (uint32_t)dst_ptr, (size_t)dst_max, 0 };
    int result = inflate_stream(in_buf, src_len, wasm_write_cb, &ctx);
    free(in_buf);
