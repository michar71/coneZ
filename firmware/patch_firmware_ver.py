...
This engraving is necessary for discerning images/partitions when OTA-upgrading,
...
#def get_program_ver( env ):
#    return (
#        env.GetProjectOption( "custom_prog_version", "1.0.0" ),
#        "custom_prog_version"
#    )


BUILDNUM_PATH = "./buildnum.txt"

def get_buildnum():
    try:
        with open(BUILDNUM_PATH, "r") as f:
            return int(f.read()) + 1
    except Exception:
        return 1

def save_buildnum(n):
    os.makedirs(os.path.dirname(BUILDNUM_PATH), exist_ok=True)
    with open(BUILDNUM_PATH, "w") as f:
        f.write(str(n))

def get_program_ver(env):
    base_version = env.GetProjectOption("custom_prog_version", "1.0.0")

    # custom_prog_append_buildnum
    bool_option = functools.partial(_get_bool_option, env)
    if bool_option("custom_prog_append_buildnum", False):
        buildnum = get_buildnum()
        save_buildnum(buildnum)
        full_version = f"{base_version}{buildnum:04d}"
    else:
        full_version = base_version

    return (full_version, "custom_prog_version+buildnum")
...
def get_project_name(env) -> Tuple[str, str]:
    """
    Determines a reasonable project name for esp_app_desc_t.project_name.
    Returns: (name, source_label)
    """
    # 1. Try custom_prog_name from platformio.ini
    name = env.GetProjectOption("custom_prog_name", None)
    if name:
        return name, "custom_prog_name"

    # 2. Try git project-relative folder
    try:
        name = (
            subprocess.check_output(["git", "rev-parse", "--show-prefix"], universal_newlines=True)
            .strip()
            .strip("/")
        )
        if name:
            return name, "git_relative_dir"
    except Exception:
        pass

    # 3. Fallback: last folder name in PROJECT_SRC_DIR
    try:
        src_path = env.get("PROJECT_SRC_DIR")
        name = Path(src_path).resolve().name
...
def _collect_app_infos(env) -> Tuple[AppInfos, AppInfos]:
    ...
    if bool_option("custom_appinfos_patch_appver", True):
        appver, appver_src = get_program_ver(env)

    ## TODO: parse var-value as False if off/false/no/0.
    if bool_option("custom_appinfos_patch_timestamp", False):
