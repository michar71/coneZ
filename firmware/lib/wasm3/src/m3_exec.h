...
// Abstract memory access — routes through PSRAM or direct DRAM
#if d_m3UsePsramMemory
// Hot-path: inline DRAM branch for first d_m3PsramDramWindow bytes, PSRAM for the rest.
// Individual loads/stores (1/2/4/8 bytes) can't straddle the 4KB-aligned boundary.
# define m3MemRead(mem, off, buf, sz) do {                                      \
    u32 _off = (u32)(off);                                                      \
    if (LIKELY(_off < d_m3PsramDramWindow))                                     \
        memcpy((buf), (mem)->dram_buf + _off, (sz));                            \
    else                                                                        \
        m3_psram_read((mem)->psram_addr + (_off - d_m3PsramDramWindow),         \
                      (uint8_t*)(buf), (sz));                                   \
} while(0)
# define m3MemWrite(mem, off, buf, sz) do {                                     \
    u32 _off = (u32)(off);                                                      \
    if (LIKELY(_off < d_m3PsramDramWindow))                                     \
        memcpy((mem)->dram_buf + _off, (buf), (sz));                            \
    else                                                                        \
        m3_psram_write((mem)->psram_addr + (_off - d_m3PsramDramWindow),        \
                       (const uint8_t*)(buf), (sz));                            \
} while(0)
// Bulk ops use split-aware helpers (may straddle the DRAM/PSRAM boundary).
# define m3MemMove(mem, dst, src, sz)    m3_split_move ((mem)->dram_buf, (mem)->psram_addr, (dst), (src), (sz))
# define m3MemSet(mem, off, val, sz)     m3_split_set  ((mem)->dram_buf, (mem)->psram_addr, (off), (val), (sz))
#else
# define m3MemRead(mem, off, buf, sz)    memcpy ((buf), m3MemData(mem) + (off), (sz))
# define m3MemWrite(mem, off, buf, sz)   memcpy (m3MemData(mem) + (off), (buf), (sz))
# define m3MemMove(mem, dst, src, sz)    memmove(m3MemData(mem) + (dst), m3MemData(mem) + (src), (sz))
# define m3MemSet(mem, off, val, sz)     memset (m3MemData(mem) + (off), (val), (sz))
#endif

// Periodic yield for PSRAM memory operations — prevents Interrupt WDT timeout
// when WASM programs do long runs of load/store with no function calls.
#if d_m3UsePsramMemory
# define d_m3PsramYield()                                   \
    if (++m3_psram_yield_ctr >= 512) {                      \
        m3_psram_yield_ctr = 0;                             \
        m3ret_t _yld = m3_Yield ();                         \
        if (UNLIKELY(_yld)) return _yld;                    \
    }
#else
# define d_m3PsramYield()
#endif
...
d_m3Op  (MemCopy)
{
    ...
    if (destination + size <= _mem->length)
    {
        ...
        {
            m3MemMove (_mem, destination, source, size);
            d_m3PsramYield ();
            nextOp ();
        }
...
d_m3Op  (MemFill)
{
    ...
    if (destination + size <= _mem->length)
    {
        m3MemSet (_mem, destination, (u8) byte, size);
        d_m3PsramYield ();
        nextOp ();
    }
    else d_outOfBoundsMemOp (destination, size);
...
{                                                       \
    ...
    if (m3MemCheck(                                     \
        ...
        SRC_TYPE value;                                 \
        m3MemRead(_mem, operand, &value, sizeof(value));\
...
{                                                       \
    ...
    if (m3MemCheck(                                     \
        ...
        m3MemWrite(_mem, operand, &val, sizeof(val));   \
        d_m3PsramYield ();                              \
        nextOp ();                                      \
    } else d_outOfBounds;                               \
}                                                       \
