...
void  Runtime_Release  (IM3Runtime i_runtime)
{
    ...
    if (i_runtime->memory.mallocated && !i_runtime->memory.mallocated->prealloc)
    {
#if d_m3UsePsramMemory
        if (i_runtime->memory.mallocated->psram_addr)
            m3_psram_free (i_runtime->memory.mallocated->psram_addr);
        if (i_runtime->memory.mallocated->dram_buf)
            m3_Free (i_runtime->memory.mallocated->dram_buf);
#endif
...
M3Result  ResizeMemory  (IM3Runtime io_runtime, u32 i_numPages)
{
    ...
    if (numPagesToAlloc <= memory->maxPages)
    {
...
#if d_m3UsePsramMemory
        // Header + DRAM window in DRAM; remaining data in PSRAM.
        {
        size_t psram_bytes = (numPageBytes > d_m3PsramDramWindow)
                           ? (numPageBytes - d_m3PsramDramWindow) : 0;

        if (memory->mallocated && memory->mallocated->prealloc)
        {
            if (numPagesToAlloc != memory->numPages)
            {
                // memory.grow: clone to a new block, don't touch the prealloc original.
                M3MemoryHeader * newHdr = (M3MemoryHeader *) m3_Malloc (sizeof (M3MemoryHeader));
                _throwifnull (newHdr);
                memset (newHdr, 0, sizeof (M3MemoryHeader));

                // Clone DRAM buffer
                uint8_t * new_dram = (uint8_t *) m3_Malloc (d_m3PsramDramWindow);
                if (!new_dram) { m3_Free (newHdr); _throw ("dram window alloc failed"); }
                memcpy (new_dram, memory->mallocated->dram_buf, d_m3PsramDramWindow);
                newHdr->dram_buf = new_dram;

                // Clone PSRAM (allocate larger, copy old data)
                if (psram_bytes) {
                    uint32_t pa = m3_psram_alloc (psram_bytes);
                    if (pa == 0) { m3_Free (new_dram); m3_Free (newHdr); _throw ("psram alloc failed"); }
                    size_t old_psram = memory->mallocated->length - d_m3PsramDramWindow;
                    if (old_psram && memory->mallocated->psram_addr)
                        m3_psram_memcpy (pa, memory->mallocated->psram_addr,
                                         old_psram < psram_bytes ? old_psram : psram_bytes);
                    newHdr->psram_addr = pa;
                }

                newHdr->length   = numPageBytes;
                newHdr->runtime  = io_runtime;
                newHdr->maxStack = (m3slot_t *) io_runtime->stack + io_runtime->numStackSlots;
                newHdr->prealloc = false;
                memory->mallocated = newHdr;
            }
            else
            {
                // Initial load matches prealloc — reuse in place, update runtime fields
        ...
        {
            if (! memory->mallocated) {
                memory->mallocated = (M3MemoryHeader *) m3_Malloc (sizeof (M3MemoryHeader));
                _throwifnull (memory->mallocated);
                memset (memory->mallocated, 0, sizeof (M3MemoryHeader));
            }

            // DRAM buffer — allocate once, keep on grow (always same size)
            if (! memory->mallocated->dram_buf) {
                memory->mallocated->dram_buf = (uint8_t *) m3_Malloc (d_m3PsramDramWindow);
                _throwifnull (memory->mallocated->dram_buf);
                memset (memory->mallocated->dram_buf, 0, d_m3PsramDramWindow);
            }

            // PSRAM — free old and allocate new (size changes on grow)
            if (memory->mallocated->psram_addr) {
                m3_psram_free (memory->mallocated->psram_addr);
                memory->mallocated->psram_addr = 0;
            }

            if (psram_bytes) {
                uint32_t pa = m3_psram_alloc (psram_bytes);
                _throwif ("psram alloc failed", pa == 0);
                memory->mallocated->psram_addr = pa;
            }
        ...
        m3log (runtime, "resized (psram) mem: %p; dram: %p; psram: 0x%08x; length: %zu; pages: %d",
               memory->mallocated, memory->mallocated->dram_buf,
               memory->mallocated->psram_addr, numPageBytes, memory->numPages);
        }
#else
        ...
        if (memory->mallocated && memory->mallocated->prealloc)
        {
            if (numPagesToAlloc != memory->numPages)
            {
                // memory.grow: clone to a new block, don't touch the original.
                size_t numPreviousBytes = memory->numPages * d_m3MemPageSize;
                size_t copyBytes = numPreviousBytes + sizeof (M3MemoryHeader);
                if (copyBytes > numBytes) copyBytes = numBytes;

                void * newMem = m3_Malloc (numBytes);
                _throwifnull (newMem);
                memcpy (newMem, memory->mallocated, copyBytes);

                memory->mallocated = (M3MemoryHeader *) newMem;
                memory->mallocated->prealloc = false;
            }
            // else: initial load matches prealloc — reuse in place
        }
        else
        {
            size_t numPreviousBytes = memory->numPages * d_m3MemPageSize;
...
#if d_m3UsePsramMemory
            {
            M3MemoryHeader * hdr = io_memory->mallocated;
            u32 seg_end = (u32)segmentOffset + segment->size;
            if (seg_end <= d_m3PsramDramWindow) {
                // Entirely in DRAM window
                memcpy (hdr->dram_buf + segmentOffset, segment->data, segment->size);
            } else if ((u32)segmentOffset >= d_m3PsramDramWindow) {
                // Entirely in PSRAM
                m3_psram_write (hdr->psram_addr + (u32)segmentOffset - d_m3PsramDramWindow,
                                segment->data, segment->size);
            } else {
                // Straddles boundary
                u32 dram_part = d_m3PsramDramWindow - (u32)segmentOffset;
                memcpy (hdr->dram_buf + segmentOffset, segment->data, dram_part);
                m3_psram_write (hdr->psram_addr, segment->data + dram_part,
                                segment->size - dram_part);
            }
            }
#else
...
#if d_m3UsePsramMemory
        // Linear memory is in PSRAM — no DRAM pointer available.
        // Host imports must use wasm_mem_* helpers instead.
        (void) memory;
#else
        if (size)
