...
- **LoRa:** RadioLib, SX1262/SX1268 via SPI (custom `EspHal` HAL in `lora/lora_hal.h` — raw GPSPI3 registers, ESP-IDF GPIO/timers, no Arduino dependency). Two modes selectable via `lora.mode` config key. **LoRa mode** (default): configurable frequency/BW/SF/CR (defaults: 431.250 MHz, SF9, 500 kHz BW). **FSK mode**: a `[fsk]` config section (`bitrate`, `freqdev`, `rxbw`, `shaping`, `whitening`, `sync` hex string, `crc`); validated end-to-end — a full 1.2 MB firmware dist OTA over GFSK at +0 dBm with **~0% loss** (1 lost chunk / ~6200, FEC-recovered). Shared params (frequency, TX power, preamble) work in both modes. CLI `lora` subcommands (`freq`, `power`, `bw`, `sf`, `cr`, `mode`) hot-apply changes without saving to config; `config set lora.*`/`fsk.*` persists but requires reboot.
  ...
  - **FSK RX-BW default = 29.3 kHz is the lossless lever (deviation is secondary):** the narrow **19.5 kHz RX BW** was the problem — it lost ~4.6% of dist chunks at 5 kHz dev *and* still lost ~0.8 chunks/block at 4800 Hz dev, while **29.3 kHz RX BW is ~lossless at BOTH 4800 Hz and 10 kHz dev** (full 1.2 MB OTA at +0 dBm: 4 and 1 lost chunks respectively / ~6200; vs ~285 at 5 kHz/19.5). So the **RX bandwidth is the lever, not the deviation** — 19.5 kHz clips the GFSK signal + skirts + ~±2 kHz TCXO offset; 29.3 kHz passes it. The 10 kHz deviation (h≈4) is kept only as a *secondary* offset-margin factor (modulation index = bigger fraction → more per-cone TCXO tolerance; a sweep showed ±3 kHz ≈ 4× errors). The SX126x has no usable RX AFC (`getFrequencyError()` is LoRa-only) and none is needed; TX power +0→+15 dBm barely moved loss (not margin-limited). Master GFSK TX is clean (SDR-verified: every burst maps to a logged TX size, flat envelope — no FIFO underrun).
