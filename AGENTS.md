...
| GPIO / I2C / OTA | `driver/gpio.h`, `driver/i2c_master.h`, `esp_ota_ops.h` — direct ESP-IDF drivers |
...
```
c2wasm compiles a C subset (int/float/double/void/char/long long, unsigned int/unsigned long long/uint8_t..uint64_t/size_t, short/signed/_Bool/bool, const enforcement, if/else/for/while/do/switch, full operator precedence, comma operator, ternary, multidimensional arrays with chained subscripts and compound assignment, nested/designated array initializers, pointer arithmetic/dereference/address-of/increment with element-size scaling (including local/global scalar linear-memory spill-slot addressing), tag-based structs with `.`/`->` member access (struct-by-value locals/globals, pointer-to-struct, arrays of struct, struct fields that are arrays or nested structs — no anonymous structs, unions, initializer lists, or pass-by-value), `#include <conez_api.h>` (angle brackets = compiler-internal; `<stdint.h>` etc. are no-ops) plus `#include "user_header.h"` for real files spliced relative to the source, `#define`/`#undef` (with line continuation), `#ifdef`/`#ifndef`/`#elif`/`#else`/`#endif`/`#if` with full constant expressions, `#error`/`#warning`, printf, host_snprintf, file I/O, malloc/free/calloc/realloc imports, WASM-native sqrtf/fabsf/floorf/ceilf/truncf/fminf/fmaxf + f64 equivalents, f64 math sin/cos/tan/etc.) directly to WASM. No clang, LLVM, or SDK needed. Performs constant folding during codegen (binary arith/bitwise/shift/compare on i32/i64/f32/f64, unary minus, with 3-slot adjacency tracking so nested literal subexpressions collapse fully). Test suite: `make test` (currently 167 tests, compile + wasm-validate) and `make test-runtime` (a node-based harness that actually executes tests with `// EXPECTED:` blocks and diffs output — currently 92 runtime tests). See `documentation/c2wasm.txt` for the full reference.
...
**Offline compiler tools:** `tools/bas2wasm/` (BASIC→WASM, currently 97 tests + 67 runtime tests via `make test-runtime`; supports QuickBASIC `'$INCLUDE: 'file.bi'` / `REM $INCLUDE:` file splicing) and `tools/c2wasm/` (C→WASM, currently 167 tests + 92 runtime tests via `make test-runtime`; `#include <conez_api.h>` is internal, `#include "file.h"` splices a real file) are self-contained compilers with no external dependencies beyond libc. Each has its own `buildnum.txt` for independent version tracking. Build each with `make` from its directory, or `make` from `tools/` to build all. Each has a `make test` target (compile-each + wasm-validate, with `reject_*` files expected to fail), a `make test-runtime` target (node-based harness that executes tests with an EXPECTED comment block and diffs the printed output — catches semantic regressions wasm-validate can't), and a `make verify` target for byte-exact baseline comparison. Both compilers also work as embedded libraries — the simulator and firmware link them directly (no subprocess spawning) via platform abstraction headers (`bas2wasm_platform.h`, `c2wasm_platform.h`). In embedded mode, `malloc`/`fprintf`/`exit` redirect to host-provided callbacks, and shared global names are macro-prefixed (`bw_`/`cw_`) via the `#define` blocks in `bas2wasm_platform.h`/`c2wasm_platform.h` to avoid link-time collisions when both compilers coexist in the same binary. **Any new non-static global added to one compiler whose name also exists in the other MUST be added to both prefix blocks** — the standalone CLIs and the firmware tolerate a missing prefix (the xtensa toolchain merges duplicate tentative definitions) but the Qt simulator links both embed libs into one binary under a strict `-fno-common` host linker and fails with `multiple definition of '<name>'`. **Build the simulator (not just firmware) after touching compiler globals** — it's the only build that catches this class of regression. Both perform codegen-time constant folding (3-slot tracking; see the compilers' docs for details). See `documentation/bas2wasm.txt` and `documentation/c2wasm.txt` for full references.
...
**Differences from firmware:** Sensors come from GUI sliders (not real hardware). GPIO stubs log to console. No LoRa, WiFi, or PSRAM. Cue engine plays back .cue files with full timeline support; when not playing, cue_playing/cue_elapsed fall back to sensor panel sliders. File I/O uses the data directory. DateTime uses host system clock. ArtNet TX available (opt-in via `--artnet-tx`/`--artnet` or `artnet tx enable`) — sends LED data as Art-Net UDP packets to drive real fixtures or visualizers. ArtNet RX available (opt-in via `--artnet-rx` or `artnet enable`) — receives Art-Net OpOutput packets and maps DMX data onto the simulated LED channels, matching the firmware ArtNet receiver. TX CLI under `artnet tx ...`; RX CLI at top-level `artnet ...` (matching firmware).
...
- **LoRa:** RadioLib, SX1262/SX1268 via SPI (custom `EspHal` HAL in `lora/lora_hal.h` — raw GPSPI3 registers, ESP-IDF GPIO/timers, no Arduino dependency). Two modes selectable via `lora.mode` config key. **LoRa mode** (default): configurable frequency/BW/SF/CR (defaults: 431.250 MHz, SF9, 500 kHz BW). **FSK mode**: a `[fsk]` config section (`bitrate`, `freqdev`, `rxbw`, `shaping`, `whitening`, `sync` hex string, `crc`); validated end-to-end — a full 1.2 MB firmware dist OTA over GFSK at +0 dBm with **~0% loss** (1 lost chunk / ~6200, FEC-recovered). Shared params (frequency, TX power, preamble) work in both modes. CLI `lora` subcommands (`freq`, `power`, `bw`, `sf`, `cr`, `mode`) hot-apply changes without saving to config; `config set lora.*`/`fsk.*` persists but requires reboot.
  - **Cone TCXO gotcha (FSK):** `radio.beginFSK(...)` runs `modSetup`→`findChip`→`reset()`, which **wipes any prior `setTCXO()`**, and RadioLib only re-applies the TCXO when `tcxoVoltage > 0`. So `beginFSK(..., tcxoVoltage=0, ...)` leaves the TCXO **off** → no reference clock → the FSK demod is dead (preamble never detected, IRQ status stuck `0x0000`). `radio.begin()` (LoRa) dodges this via its 1.6 V default. **Always pass the real TCXO voltage (1.8) to `beginFSK`** — `lora_setup()` and `lora_reinit()` both do.
  - **FSK RX-BW default = 29.3 kHz is the lossless lever (deviation is secondary):** the narrow **19.5 kHz RX BW** was the problem — it lost ~4.6% of dist chunks at 5 kHz dev *and* still lost ~0.8 chunks/block at 4800 Hz dev, while **29.3 kHz RX BW is ~lossless at BOTH 4800 Hz and 10 kHz dev** (full 1.2 MB OTA at +0 dBm: 4 and 1 lost chunks respectively / ~6200; vs ~285 at 5 kHz/19.5). So the **RX bandwidth is the lever, not the deviation** — 19.5 kHz clips the GFSK signal + skirts + ~±2 kHz TCXO offset; 29.3 kHz passes it. The default 9.6 kHz deviation (h=4.0, a clean integer modulation index; bench-validated at the RF-equivalent 10 kHz) is kept only as a *secondary* offset-margin factor (bigger deviation → the offset is a smaller fraction → more per-cone TCXO tolerance; a sweep showed ±3 kHz ≈ 4× errors). The SX126x has no usable RX AFC (`getFrequencyError()` is LoRa-only) and none is needed; TX power +0→+15 dBm barely moved loss (not margin-limited). Master GFSK TX is clean (SDR-verified: every burst maps to a logged TX size, flat envelope — no FIFO underrun).
  - **v1 beacon format (`lora_proto.h`/`.py`, protocol doc §10):** the beacon `mode` byte is a **flags** field (bit0 = mode, bit1 = FSK whitening); the 8-byte PHY region is a **mode-keyed union** — LoRa `bw(u32)/sf/cr/sync` or FSK `bitrate/freqdev/rxbw/sync` as **u16 ÷100** (multiples of 100, so no cap → 6.5 Mbps/MHz). The cone's beacon line is **mode-aware**: FSK shows `bitrate/dev/rxbw/sync/whiten` and **omits SNR** (the SX126x has no FSK SNR — it's a stale LoRa value). FSK sync default = **0xDEAD** (matches the LoRa sync). **Whitening is default-on** (scanlist `FW` flag → `config.fsk_whitening` → `setWhitening(true, 0x01FF)`; master `setFskWhitening(0x01FF)` — seeds must match, fixed at RadioLib's `0x01FF`). Scanlist flags loop-parse after the mode char: `X`=receive-only, `W`=FSK whitening (`FW`/`FWX`). **Reconfiguration handoff:** the beacon also doubles as the master's channel-change announcement — before retuning, the master sends the new params on the OLD PHY 10× so locked cones FOLLOW (`scan_notify_beacon`→`scan_phy_eq`/`scan_apply` in `scan.cpp`; excludes `tx_power`/CR) instead of rescanning. The cone also keeps the **last-connected channel** (`last_conn`) and probes it **1-in-`LAST_CONN_EVERY`** (=3) scan steps during a rescan, so it re-finds a master that moved to a non-scanlist channel; it's **persisted to `/scanlast.txt`** (one scanlist-format line, compare-before-write, re-seeded at boot via `scan_load_last_conn`), so the re-find survives a power cycle. See protocol doc §15. **Network id filter:** the 4-byte header's network byte (`LP_HDR_NET`) is enforced — `lora_rx()` drops any packet whose id != `config.lora_network` (`[lora] network`, 0-255, default 0) before dispatch, and the master stamps its `[lora] network` into every header (`make_header`/`make_beacon`/dist chunks). Lets two ConeZ networks share one sync word without cross-talk. (The old unused `lora_ssid` config field was removed.)
...
- **HTTP/OTA:** ESP-IDF `esp_http_server` on port 80 (`http/http.cpp`). Runs in its own FreeRTOS task (pinned to core 1, stack 4096) — no polling needed. Root page shows GPS, partition info, and links to `/config`, `/dir`, `/nvs`, `/update`, `/reboot`. Config form submits URL-encoded POST body, parsed with `httpd_query_key_value()` + `url_decode()`. OTA firmware/filesystem upload at `/update` — HTML form uses JavaScript `fetch()` for raw binary POST (no multipart parsing), `type` passed as query param. CLI-friendly via curl:
  ...
  ```
  Uses ESP-IDF `esp_ota_ops.h` + `esp_partition.h`; serialized against the LoRa dist OTA via `ota_lock` (same inactive slot). **Firmware** path: `esp_ota_begin(OTA_WITH_SEQUENTIAL_WRITES)` → write → `esp_ota_end` (verifies) → `set_boot_partition`. **Filesystem** path is hardened so a bad upload can't brick the FS: (1) **length check first** — a LittleFS image must be *exactly* the partition size (block count derives from it), rejected up front while the FS is still mounted/intact; (2) `esp_vfs_littlefs_unregister()` (clears `littlefs_mounted` so config/cue/dist skip the gone FS); (3) **lazy per-sector erase** — each 4 KB sector erased just before its write, never one big up-front `esp_partition_erase_range(0, size)` that would block the httpd task for seconds → WDT; (4) **read-back MD5 verify** — re-reads the partition and compares to the streamed hash before rebooting; on mismatch it does *not* reboot (FS left unmounted, operator re-uploads). The handler keeps its 1 KB bounce buffer + MD5 state in a heap-`malloc`'d scratch (DRAM — passed to `httpd_req_recv`/`esp_ota_write`/`esp_partition_read+write` by address, and PSRAM here isn't memory-mapped) freed on every exit, so the httpd stack stays 4 KB. Auto-reboots on success. Progress logged via `printfnl(SOURCE_SYSTEM, ...)`.
- **MQTT:** ESP-IDF `esp_mqtt_client` in `mqtt/conez_mqtt.cpp/h`. Connects to the sewerpipe broker over WiFi with auto-reconnect (built into esp_mqtt). Publishes JSON heartbeats every 30s to `conez/{id}/status` with uptime, heap, temp, RSSI. Debug messages are forwarded to `conez/{id}/debug` by printManager (SOURCE_MQTT excluded to prevent loops). Subscribes to `conez/{id}/cmd/#` for per-cone commands. The esp_mqtt task runs on core 1 (`CONFIG_MQTT_USE_CORE_1` in sdkconfig.defaults). Config section `[mqtt]` with `broker` (default: `sewerpipe.local`), `port` (default: 1883), and `enabled` (default: on) keys. CLI: `mqtt` (status), `mqtt enable`/`disable`, `mqtt connect`/`disconnect`, `mqtt broker <host>` (hot-apply), `mqtt pub <topic> <payload>`. Debug output via `SOURCE_MQTT` (`[debug] mqtt` / `DEFAULT_DBG_MQTT`, default **off** — the broker is often absent on the bench and would spam connect-error lines). `mqtt_publish()` is thread-safe (esp_mqtt uses a recursive mutex). See `documentation/mqtt.txt` for topic hierarchy and protocol details.
- **ArtNet (receiver):** LWIP UDP socket in `led/artnet.cpp/h`. Receives Art-Net OpOutput (opcode `0x5000`) packets on UDP port 6454 and writes DMX data directly into `leds1..4`, then calls `led_show()`. Each LED channel is independently mapped to a configurable (universe, DMX start address) pair — channels mapped to the same universe are all updated from a single received packet. DMX addresses are 1-indexed (standard convention); setting `dmxN = 0` disables that channel. The task opens its socket after WiFi connects and reopens it automatically after a reconnect. Config section `[artnet]` keys: `enabled` (bool, default off), `uni1`–`uni4` (ArtNet universe per LED channel, default 0), `dmx1`–`dmx4` (DMX start address 1-512 per channel; default dmx1=1, dmx2-4=0/disabled). `artnet_setup()` is called in `main.cpp` after `mqtt_setup()` and `printManagerInit()`. CLI: `artnet` (status + channel mapping), `artnet enable`/`disable` (toggle + save config), `artnet start`/`stop` (transient), `artnet universe <ch> <n>` / `artnet dmx <ch> <addr>` (hot-apply mapping, not saved). Use `config set artnet.*` to persist mapping across reboots.
- **CLI:** ConezShell (`util/shell.cpp/h`) on DualStream — both USB Serial and Telnet (port 23) active simultaneously, all output to both. TelnetServer (`console/telnet.cpp/h`) uses BSD sockets (`lwip/sockets.h`) with non-blocking I/O, supports up to `TELNET_MAX_CLIENTS` (8) simultaneous clients with per-slot IAC state and a 30-minute idle reap, bare `\n` → `\r\n` translation on output, prompt delivery on connect, and Ctrl+D per-session disconnect. **Never use `ioctl(fd, FIONREAD, …)` on these sockets** — lwIP compiles the `FIONREAD` case out unless `LWIP_SO_RCVBUF` or `LWIP_FIONREAD_LINUXMODE` is set, and `CONFIG_LWIP_SO_RCVBUF` defaults to `n`. The call fails every time; `available()` then returns 0, `DualStream::read()` never reaches the telnet branch, and **typed input is silently discarded while output keeps working perfectly** (output uses `send()`, which never touches `ioctl`). Input readiness is determined with a non-blocking `recv(..., MSG_PEEK | MSG_DONTWAIT)` instead (`slot_pending()`), and `read()` lets the `O_NONBLOCK` `recv()` itself report "nothing pending" via `EAGAIN`. Arrow keys, Home/End/Delete, Ctrl-A/E/U, 32-entry command history (PSRAM-backed ring buffer on ConeZ PCB, single-entry DRAM fallback on Heltec). ANSI color output on by default, toggleable at runtime via `color on`/`color off` CLI command (`setAnsiEnabled()`/`getAnsiEnabled()` in `printManager.h`). Commands requiring ANSI (art, clear, game, winamp) error out when color is off; editor falls back to a line-based mode. File commands auto-normalize paths (prepend `/` if missing) via `normalize_path()` in `main.h`. Full-screen text editor (`console/editor.cpp/h`) for on-device script editing, with line-editor fallback when ANSI is disabled. See `documentation/cli-commands.txt` for the full command reference.
...
- **NTP** (time_source=1, ~10-50ms accuracy) — Any board with WiFi. Uses ESP-IDF `esp_sntp` directly (`esp_sntp_setoperatingmode`/`esp_sntp_setservername`/`esp_sntp_init`). Auto-initializes in `ntp_loop()` when WiFi connects (no need to call `ntp_setup()` from WiFi commands). Re-syncs periodically (`config.ntp_interval`, default 3600s). NTP server configurable via `[system] ntp_server` config key. Only `ntp_sync_cb()` promotes `time_source` to 1 — `ntp_loop()` updates epoch from `gettimeofday()` but does not change `time_source` (the RTC survives soft resets and would falsely indicate NTP).
...
**WARNING — ADC/SPI pin conflict:** PSRAM uses GPIO 4-7 for SPI (CE/MISO/SCK/MOSI). LoRa uses GPIO 8-10. The ESP-IDF ADC driver's `adc_oneshot_config_channel()` calls `gpio_config_as_analog()` which permanently disables GPIO input/output on the pin, silently breaking SPI communication. **Never configure ADC on GPIO 4-10.** Only GPIO 1-3 (ADC1 channels 0-2) are safe for ADC reads. The `adc_setup()` function in `sensors/adc.cpp` only initializes these safe channels.
...
INI-style config file (`/config.ini`) on LittleFS, loaded at boot. Descriptor-table-driven: `cfg_table[]` in `config.cpp` maps `{section, key, type, offset}` to `conez_config_t` struct fields. Covers WiFi, GPS origin, LoRa radio params (both LoRa and FSK modes), MQTT broker, NTP server, timezone, LED counts and default colors, ArtNet receiver mapping, debug defaults, callsign, and startup script. CLI commands: `config`, `config set`, `config unset`, `config reset`. See `documentation/config.txt` for full reference.
...
**The master node does not run sewerpipe — it runs Mosquitto.** Cones only require *a* MQTT 3.1.1 broker reachable at `sewerpipe.local:1883`; sewerpipe and Mosquitto are interchangeable from their point of view. Mosquitto 2.x needs three non-default settings, all documented in `documentation/mqtt.txt`: a `listener 1883 0.0.0.0` (it otherwise binds localhost-only), `allow_anonymous true` (the firmware connects anonymously by default; it *can* do username/password auth via `mqtt.username`/`mqtt.password`, but no cone sets them out of the box, so an auth-requiring broker rejects the fleet until each cone is provisioned), and a second `listener 9001` with `protocol websockets` for browser dashboards. **Every cone also needs a unique `system.cone_id`** — client id *and* topics derive from it, `DEFAULT_CONE_ID` is 0, and a colliding fleet evicts itself from the broker in a reconnect storm (which is what exposed the blocking-publish hang that froze `loopTask` on every cone). A `sewerpipe-alias.service` systemd unit publishes the `sewerpipe.local` mDNS name, since the Pi's hostname is `raspberrypi` and the firmware hardcodes `DEFAULT_MQTT_BROKER "sewerpipe.local"`. Net effect: **the broker ships open (anonymous, no TLS); `cmd/#` topics are the full device CLI, so on anything but a controlled show LAN, enable auth — provision every cone first, then require it broker-side (order + steps in `documentation/mqtt.txt`).**
...
Python LoRa master node that runs on a Raspberry Pi (SX1262/SX1268 via the bundled `LoRaRF` driver). It is the downstream broadcaster for over-the-air distribution of time beacons, manifests, scripts/data files, and firmware images to ConeZ cones in the field. The **v1 protocol** (`documentation/lora-protocol.txt`) is being built in phases (`documentation/lora-v1-implementation-plan.txt`): v1 framing + beacon (Phase 1), scanlist + channel lock (Phase 2 — tiered round-robin with an exhaustive fallback sweep, streamed open-per-step from LittleFS, RX-only-channel aware, plus `lora on/off`, `firmware/src/lora/scan.cpp`), and `dist` file distribution (Phase 3 core — eventual-consistency carousel, `firmware/src/lora/dist.cpp`; Phase 4 — per-block deflate; Phase 5 — per-block systematic Reed-Solomon FEC, `firmware/src/lora/rs.c` ↔ `lora-master/reed_solomon.py`; Phase 6 — firmware OTA: firmware is a dist entry streamed to the inactive OTA partition, product/version-matched, downgrade-protected, `esp_ota_set_boot_partition`+reboot — **requires `CONFIG_SPI_FLASH_AUTO_SUSPEND`**, see below) are implemented; cues and the registration/polling control plane are still to come.

**Components:**
- `conez-master.py` — emits the **v1 BEACON** every 10 s; **broadcasts the manifest on its own timer** (`[dist] manifest_interval`, default 60 s — a freshly-built/reloaded manifest is sent promptly regardless), and runs a **WEIGHTED dist carousel** (spec §13.7; `build_dist_chunks()`/`send_manifest()`/`send_next_payload_chunk()`) that between manifests holds a data-file:firmware chunk ratio of `[dist] file_per_fw`:1 — a **float** (default **1** = even; >1 favours files so small files stay responsive while a big image trickles, <1 favours firmware to push an image faster): each data file AND firmware image (`parse_manifest_firmware`, `firmware/<product>/<version>.bin`) as per-block chunks — N DATA chunks (deflated per block when it shrinks the file, `proto.encode_file`) + R Reed-Solomon PARITY chunks (`reed_solomon.encode`). The beacon carries version, master epoch, channel descriptor, manifest serial, and callsign. Auto-selects the highest-numbered manifest at startup via `latest_manifest()` and **hot-reloads** a newer `manifest_<N>.txt` live — checked once per beacon (`reload_manifest_if_changed()`), it rebuilds the chunk set first and only swaps on success, so a freshly-run `build_manifest.py` is picked up within ~10 s with no restart (and a half-written manifest can't crash the master). Downstream defaults: 431.250 MHz, 500 kHz BW, SF7, CR 4/5, sync word 0xDEAD, +0 dBm (bench default — raise for field range). **Bench knobs (env):** `DIST_TEST_DROP_DATA=K` drops the last K data chunks/block (parity still sent) to force RS-recovery; `DEADTIME=<s>` shrinks the inter-packet gap to speed bench transfers. **Config + control:** at startup it loads an optional `conez-master.ini` (`load_config()`, see `conez-master.ini.example`) that overrides the hard-coded defaults — radio params, beacon period, deadtime, callsign, dist dirs, `manifest_interval` (60 s) + carousel `file_per_fw` weight, control-socket path — and serves a **Unix control socket** (`[control] socket`, default `/tmp/conez-master.sock`) in a daemon thread. **`reload` is deterministic**: every setting is recomputed as `_CFG_DEFAULTS` (hard-coded defaults snapshotted once at import) ← file ← env, so *deleting* a key from the file reverts it to the default rather than carrying over the last reloaded value. Radio setup is wrapped in `apply_radio_config()` so a live `reload` can re-apply it (standby → set → RX). When a `reload` changes the **PHY** (any radio param except `tx_power`/LoRa CR), `do_full_reload()` first **announces** the new params — 10 beacons on the OLD PHY, 0.25 s apart, each rebuilt with a live timestamp (`_announce_radio_change`) — then retunes, so locked cones **follow** to the new channel without dropping lock (cone: `scan_notify_beacon` follow path in `scan.cpp`; bench-validated 431.250↔431.500, ~38 ms to follow, **0 rescans** vs the old ~40-90 s loss→rescan). `tx_power`/CR changes apply silently (no announce burst). Control commands set flags the **main loop** acts on, so config/manifest/radio reloads never race a transmit.
...
- `LoRaRF/` — bundled SX126x/SX127x Python driver library. **Locally patched — do not blindly re-vendor upstream.** `SX126x.py` used to swap the DIO1 interrupt handler on every mode change (`remove_event_detect()` + `add_event_detect()` in `endPacket()`, `request()`, and `listen()`). On Debian Bookworm, `RPi.GPIO` is the **`rpi-lgpio` shim**, whose `remove_event_detect()` cancels the Python callbacks but *does not release the underlying lgpio alert claim* (see its own `_unset_alert()` docstring). The paired `add_event_detect()` therefore re-claimed an already-claimed line on every transmit; lgpio tolerated a few hundred of those and then rejected the claim with `lgpio.error: 'bad event request'`, killing the master mid-run (observed at ~1900 transmits, ~8 min). Fixed by arming the IRQ **once** (`_armIrq()`) with a permanent handler (`_interruptDispatch()`) that routes to the TX/RX/RX-continuous handler based on `_statusWait` — which every caller already sets *before* arming, so handler selection is unchanged. `_irqArmed` is reset in `end()` (after `gpio.cleanup()`) and in `setPins()` when the IRQ line changes. Upstream LoRaRF still has the churn; it is latent on real `RPi.GPIO` and fatal on `rpi-lgpio`.
  ...
  **`wait()` is bounded — and must stay that way.** Arming once removed the crash but exposed a hang: in interrupt mode `wait()` spun on `_statusIrq`, a flag only the DIO1 callback sets, and with its default `timeout=0` the timeout check could never fire. It never read the radio. A single missed edge therefore blocked the caller forever — the master fell silent mid-transmit (log ends after `TX Len`/`ASCII` with no `TX start`) while the process stayed alive and still answered `masterctl status`, which reported `tx=enabled`. Observed after ~4 h / ~57k transmits. The per-transmit `remove`/`add` churn had been accidentally re-arming a lost alert, masking this. Now `wait()` bounds the wait (`IRQ_WAIT_TIMEOUT_S`), then reads the IRQ status register — authoritative whether or not the edge arrived — and completes normally if the radio finished. If it genuinely hasn't, `_rearmIrq()` reinstalls the handler and `wait()` returns `False` so the caller resynchronises. `_rearmIrq()` calls `remove_event_detect()` **before** `add_event_detect()`, or the shim appends a second callback to the existing alert and every edge fires the dispatcher twice; it is a recovery path only and must never move back onto the per-transmit path, which would resurrect the `bad event request` crash. `conez-master.py` logs `WARN: TX wait timed out` when this fires — a rising rate means DIO1 edges are being lost. Note `wait(timeout=0)` now means "up to `IRQ_WAIT_TIMEOUT_S`", not "forever".
...
- `conez-master.ini.example` — documents every overridable setting; copy to `conez-master.ini` (gitignored) per deployment.

**Workflow to publish a new firmware/file set:** drop the file(s) into `lora-master/firmware/<product>/` or `lora-master/dist/`, run `python3 build_manifest.py` to regenerate the manifest (written to `dist-state/`). If `conez-master.py` isn't running, start it (it picks up the newest manifest automatically); if it's **already running, it hot-reloads** the new manifest within a beacon — no restart needed (or force a config+manifest reload with `conez-masterctl reload`).

**Runbook — pushing a firmware image to the fleet over LoRa** (as done on the Pi at `/root/conez/`):

1. **Bump the version.** The cone installs the image only if the manifest version is *strictly newer* than what it runs (`version_cmp` in `dist.cpp`; unless `lora.ota_downgrade` is set). A rebuild auto-increments `firmware/buildnum.txt`, so `0.02.0549` → `0.02.0550`. Deploying the *same* version the fleet already runs is a no-op — the cone marks it satisfied and skips — so it only burns airtime. (Genuinely useful when field cones are behind; pointless against an already-current fleet.)
2. **Name it right.** Copy the build to `firmware/<product>/<version>.bin` where `<product>` is the cone's `custom_prog_name` (**`ConeZ-v0`** for `conez-v0-1`) and `<version>` is the exact embedded version string (the filename stem *becomes* the advertised version). e.g. `scp firmware.bin root@pi:/root/conez/firmware/ConeZ-v0/0.02.0550.bin`.
3. **Keep exactly one `.bin` per product.** The cone takes only the first `[firmware]` entry, so **remove the old `<version>.bin`** or the manifest becomes ambiguous.
4. `python3 build_manifest.py`, then wait ~10 s for the hot-reload (or `conez-masterctl reload`). `conez-masterctl status` then shows a non-zero `fw` chunk count.
5. **Optional — go faster.** A 1.2 MB image over LoRa (SF7/500 kHz) is ~15–30 min for the fleet. Favour firmware in the carousel with `[dist] file_per_fw = 0.25` (1 file : 4 fw) in `conez-master.ini` + `conez-masterctl reload`. Reset to `1` (or pull the `.bin`) once the fleet is current, or it keeps spending airtime on an image nobody wants.

**Verify from the cone side** with the `dist` CLI command: each entry shows `ok` (have it / not newer), `WANTED` (newer — downloading), or `done`. A firmware entry flipping `ok`→`WANTED` after a version bump is the signal it will flash; `ok` means downgrade-protection correctly skipped it. The cone RS-recovers missing chunks, writes the inactive OTA slot, and reboots into the new image (rollback-confirmed on second boot).

**Running it on the Pi:** the deployment lives in `/root/conez/` — hand-copied files, not a checkout, so it drifts from this repo unless someone syncs it. It is started in a detached `screen` session (`screen -L -Logfile conez-master.log -dmS conez ...`) and **has no systemd unit, so it does not survive a reboot.** Mosquitto and `sewerpipe-alias.service` do. After a power cycle you therefore get a working broker and healthy cones with no LoRa beacon at all — which presents as a radio fault rather than a missing process. Check `pgrep -f conez-master.py` before suspecting the radio.

**A hung master still answers `status`.** The control socket is serviced independently of the TX loop, so `conez-masterctl status` can cheerfully report `tx=enabled` while nothing has been transmitted for an hour, and `stop` will be accepted and never acted on. Don't trust `status` alone — compare `grep -c 'TX start:' conez-master.log` across a few seconds, or check that the log's last packet has a `TX start` line following its `TX Len`/`ASCII` lines. (The specific hang that did this is fixed; the diagnostic property remains.)

**Controlling a running master:** `conez-masterctl status|reload|enable|disable|stop` over the Unix control socket — no restart. Edit `conez-master.ini` then `conez-masterctl reload` to apply config (incl. radio param) changes live; `disable`/`enable` toggle LoRa TX while keeping the process up. (Note: changing radio params via `reload` will desync already-locked cones until they re-scan — that's expected; managed channel/mode switching is Phase 9.)
...
**Radio bring-up gotchas (Waveshare SX1262 hat):** the first working downstream link (2026-06-20) required: RF switch via DIO2 (`setDio2RfSwitch`, txen/rxen=-1, do *not* drive TXEN); **no** DIO3 TCXO control (enabling it hangs TX); TX power set from a per-power PA table (`set_tx_power()` — LoRaRF's `setTxPower()` no-ops for SX1262 below +14 dBm → no RF); and **LDRO computed from symbol time** (`(2**SF/BW)>0.016`), not hard-coded — a mismatch with the cone's auto-LDRO makes every packet fail CRC at SF7/BW500. See `documentation/lora-protocol.txt` §16 for the full writeup.

See `documentation/lora-protocol.txt` for the **v1 protocol design** (clean-slate): master-scheduled superframe, 1-byte addressing + groups, lean 4-byte header, beacon/registration/polling/cue packet types, the `dist` eventual-consistency file/firmware distribution (per-block deflate + systematic Reed-Solomon FEC, weighted carousel, firmware-OTA-aware), scanlist-based channel discovery, and LoRa+FSK modes. Appendix A has the hard-won RF bring-up notes; Appendix B tracks built-vs-designed (Phases 1-6 plus the Phase 9 FSK PHY built so far — master GFSK TX, cone GFSK RX, and firmware dist OTA over FSK all validated on the bench; the phase map is in `documentation/lora-v1-implementation-plan.txt`).

**dist firmware OTA gotcha (Phase 6):** a cone flashing its inactive OTA partition over LoRa does `esp_partition` erase/write while the radio + system run; those disable the flash cache and cross-core-stall the other core, and on the ConeZ board that stall intermittently failed to park the other core — stretching the cache-off window to seconds → `TG1WDT_SYS_RST` or a Core-1 "cache disabled but cached memory accessed" panic, killing the multi-hour transfer. Fix = **`CONFIG_SPI_FLASH_AUTO_SUSPEND=y`** (Winbond W25Q64 `0xEF4017` supports erase-suspend), which makes flash ops take a mutex instead of disabling the cache. Set in all three sdkconfigs; flash the bootloader too when first enabling (`pio run -t upload` writes 0x0/0x8000/0x10000). It was NOT DFS, light-sleep, WiFi, PSRAM, or app-task concurrency (all ruled out on the bench). **Why only OTA surfaced it** (LittleFS/config/log writes use the same cache-disable path but almost never hang): the stall is a rare *per-erase* event and OTA is the worst case on both axes — ~300 back-to-back sector erases (vs a few for a config save) fired while the radio/inflate/RS/log-ring keep the other core hot (vs LittleFS writes at idle from the CLI). OTA samples the rare event hundreds of times under peak load; sparse low-load writes essentially never do. AUTO_SUSPEND changes the lock strategy for **all** flash ops, so it hardens LittleFS writes too — the general lesson: any sustained flash-write burst under load risks the cache-disable cross-core stall.

**dist OTA concurrency/lifecycle safeguards** (`src/util/ota_lock.*`): the HTTP firmware/filesystem upload (`http.cpp`) and the LoRa dist firmware OTA (`dist.cpp`) target the same inactive slot, so both take a single non-owning `ota_lock` (test-and-set; either may release) — whoever loses fails fast ("OTA busy") rather than interleaving erase/write into a corrupt/unbootable image. An HTTP filesystem upload clears `littlefs_mounted` so other subsystems (config/cue/dist file writes) skip the unmounted FS. In-progress dist buffers (16 KB fw + ~16 KB staged block, or up to 256 KB for a file) are freed via `dist_abort()` on `lora off` and via `dist_tick()`'s stall timeout (no dist traffic for 5 min ⇒ master gone) instead of being orphaned until reboot; brief gaps keep the buffers and resume. A `dist_mtx` serializes the chunk handler against cross-task aborts. Manifest `block_size`/chunk counts are clamped (unauthenticated link). **Compare-before-write:** each firmware sector is read and skipped if the flash already holds those exact bytes (logs `already matches` instead of `committed`), so a re-fetch after a reset/abort re-commits as cheap reads — no flash wear, no cache-off window. **Atomic file write:** dist *files* are written to `/dist/.incoming` then `rename()`d over the target (atomic on LittleFS), so a power loss mid-write never truncates the existing copy. **Debug category:** all dist chatter logs under `SOURCE_LORA_DIST` (tag `[LORA_DIST]`, `debug lora_dist on|off`, default on) — separate from general `[LORA]` beacon traffic. (The dead `SOURCE_FSYNC` category — the pre-`dist` name, never emitted — was removed.)
