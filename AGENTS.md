...
**The master node does not run sewerpipe — it runs Mosquitto.** Cones only require *a* MQTT 3.1.1 broker reachable at `sewerpipe.local:1883`; sewerpipe and Mosquitto are interchangeable from their point of view. Mosquitto 2.x needs three non-default settings, all documented in `documentation/mqtt.txt`: a `listener 1883 0.0.0.0` (it otherwise binds localhost-only), `allow_anonymous true` (mandatory — `conez_mqtt.cpp` sets only `credentials.client_id`, never a username/password, so an auth-requiring broker rejects every cone), and a second `listener 9001` with `protocol websockets` for browser dashboards. **Every cone also needs a unique `system.cone_id`** — client id *and* topics derive from it, `DEFAULT_CONE_ID` is 0, and a colliding fleet evicts itself from the broker in a reconnect storm (which is what exposed the blocking-publish hang that froze `loopTask` on every cone). A `sewerpipe-alias.service` systemd unit publishes the `sewerpipe.local` mDNS name, since the Pi's hostname is `raspberrypi` and the firmware hardcodes `DEFAULT_MQTT_BROKER "sewerpipe.local"`. Net effect: **the broker is open (anonymous, no TLS) by necessity — keep it on a controlled show LAN.**
