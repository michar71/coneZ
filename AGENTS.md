...
Python LoRa master node that runs on a Raspberry Pi (SX1262/SX1268 via the bundled `LoRaRF` driver). It is the downstream broadcaster for over-the-air distribution of time beacons, manifests, scripts/data files, and firmware images to ConeZ cones in the field. The **v1 protocol** (`documentation/lora-protocol.txt`) is being built in phases (`documentation/lora-v1-implementation-plan.txt`): v1 framing + beacon (Phase 1), scanlist + channel lock (Phase 2 — tiered round-robin with an exhaustive fallback sweep, `firmware/src/lora/scan.cpp`), and `dist` file distribution (Phase 3 — eventual-consistency carousel, `firmware/src/lora/dist.cpp`) are implemented; per-block compression/FEC, firmware OTA, cues, and the registration/polling control plane are still to come.

**Components:**
- `conez-master.py` — emits the **v1 BEACON** every 10 s (beacon-only loop; the earlier blind file/firmware/manifest carousel is retired pending the v1 `dist` rewrite — dead helpers remain dormant). The beacon carries version, master epoch, channel descriptor, manifest serial, and callsign. Auto-selects the highest-numbered manifest at startup via `latest_manifest()`. Downstream defaults: 431.250 MHz, 500 kHz BW, SF7, CR 4/5, sync word 0xDEAD, +0 dBm (bench default — raise for field range).
- `lora_proto.py` — shared v1 wire format (header + packet layouts), kept byte-for-byte in sync with `firmware/src/lora/lora_proto.h`.
- `build_manifest.py` — generates the distribution manifest. Walks `firmware/` for `*.bin` and `dist/` for files, MD5-hashes each, assigns content-stable file IDs (reused while content is unchanged), and writes a new `dist-state/manifest_<serial>.txt` only when contents change. Counter state in `dist-state/serial.txt`. The manifest/serial live in `dist-state/` (not `dist/`) so `dist/` contains only what gets distributed to the herd.
- `LoRaRF/` — bundled SX126x/SX127x Python driver library.
- `dist/` — distribution payload only (test `.bas` scripts, `scanlist.txt` channel plan in the v1 `L`/`F` per-line format — see spec §15, ...). `dist-state/` holds the generated manifest + serial counter, kept out of `dist/`.
